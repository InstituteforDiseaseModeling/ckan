...
class basepage(object):
    ...
    def __init__(self, context):
        ...
        self.url = urljoin(urljoin(self.server if context.server is None else context.server,
                                    self.relative_url if context.relative_url is None else context.relative_url),
                                    self.params if context.params is None else context.params)
    ...
    def visit(self):
        if self.driver.current_url is not None and self.driver.current_url != self.url:
            ...
            if elem in self.locator_dictionary.keys():
                try:
                    element = WebDriverWait(self.driver, self.timeout).until(
                        EC.presence_of_element_located(self.locator_dictionary[elem])
                        and EC.visibility_of_element_located(self.locator_dictionary[elem])
                    )
